...
随着支付功能接入，所有支付相关代码必须遵守以下规范。

### 11.1 回调签名验证

所有支付回调必须先验证签名，签名失败返回 HTTP 400，不执行任何业务逻辑。

### 11.2 金额校验

回调中的支付金额必须与订单 `amount` 字段比对，不一致时拒绝处理并记录审计日志。

### 11.3 幂等处理

支付回调可能重复送达（最多 7 次）。处理函数必须检查订单当前状态，已支付则直接返回成功，不重复执行业务逻辑。

### 11.4 密钥存储

支付密钥（appid/appsecret）不得出现在代码、日志、git 历史中。通过管理后台 settings 表或环境变量配置，代码只引用配置键名。

### 11.5 订单超时

未支付订单设置 TTL（默认 30 分钟）。前端轮询超时后停止查询，后端不自动取消（由管理员处理）。

## 12 会员与配额（强制）

会员系统已上线，所有涉及资源分配的操作必须遵守以下规范。

### 12.1 配额校验入口

所有创建实例、启动实例的操作必须校验会员状态和配额上限（`max_instances`、`max_running`）。校验逻辑统一在 `membership.ts` 中，路由层调用。

### 12.2 到期处理

会员到期后实例不立即删除，改为停止状态，保留数据 N 天（默认 7 天）。到期前 3 天发送提醒邮件。

### 12.3 体验会员限制

体验会员（`trial`）默认配额：`max_instances=1`、`max_running=1`、有效期 7 天、不可续费。体验会员用尽后（`trial_used=1`）不可再次领取。

### 12.4 管理员豁免

`admin`/`root` 角色不受会员状态和配额限制，可创建和运行任意数量实例。

### 12.5 后台到期检查

除请求时同步检查外，增加定时任务（每小时）扫描到期会员，批量执行：停止实例 → 更新会员状态 → 发送通知邮件。避免每次请求都扫描全表。
...
| P1 | 配置读取统一化（db.ts/email.ts/paths.ts 改为通过 config 单例） | §4.2 |
| P2 | 拆分 api.ts / pages.ts 为 routes/* | §2 |
| P2 | 拆分 views/layout.ts 为 layout/styles/components | §2 |
| P2 | 拆分 supervisor.ts | §7.1 |
| P3 | HTTP 代理请求端改流式转发 | §5.1 |
| P3 | 会员到期后台定时处理 | §12.5 |
| P3 | Schema 迁移版本化 | §6.1 |
| P3 | 实例状态机形式化 | §7.2 |
